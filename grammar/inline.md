...
PaperML 用 `%` 作注释符号（**Sprint 9.18 引入**，之前是 `//`）。
...
- 开始 `%%` 必须在行首（或缩进后空白），闭合 `%%` 可在任意位置
- 中间内容（含换行）全部被忽略
- 未闭合的块注释 → 忽略到文件末尾（不报错）
- **编辑器快捷键（Sprint 9.19）**：在行首敲第二个 `%` 时自动展开成骨架——
  ...
  光标停在中间 tab 缩进行，可直接写注释内容。

### 行内 `%%`（前面有非空白）

```
hello %% world  ← 第一个 % 触发行注释，整行变成 "hello"
next line
```

如果 `%%` 前面有**非空白字符**（比如 `hello`），那 `%%` 不被识别为块注释开始，而是当作单个 `%` 触发行注释——整行从第一个 `%` 起被吃。

### 转义

| 写法 | 含义 |
|---|---|
| `@%` | 字面 `%`（用于段落正文里出现 `%` 时；行首 `%` 不需要转义也行，因为它已经被注释吃掉） |
